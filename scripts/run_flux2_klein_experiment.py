"""
Inpainting experiment script using Flux2KleinInpaintPipeline (FLUX.2-klein).

Downloads projection outputs from Cloudflare R2, runs Flux2KleinInpaintPipeline
for all seeds across multiple GPUs, and uploads results back to R2.

Output layout per image:
    <output_root>/<dataset>/<geom_subdir>/<model_slug>_vanilla/<index>/
        seed_<seed>.png
...
Single GPU, single seed:
    ...
    python scripts/run_flux2_klein_experiment.py \\
        --dataset mirrorbench_v2 \\
        --geom-subdir estimated_geometry \\
        --output-root /tmp/flux2_klein_experiment \\
        --seeds 0
...
def _load_pipeline(gpu_id: int) -> Flux2KleinInpaintPipeline:
    device = f"cuda:{gpu_id}"
    pipe = Flux2KleinInpaintPipeline.from_pretrained(MODEL_NAME, torch_dtype=torch.bfloat16)
    pipe.to(device)
    return pipe
...
def _worker(
    ...
    model_slug = MODEL_NAME.replace("/", "--") + "_vanilla"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run FLUX.2 Klein inpainting experiment from R2 projection outputs")
