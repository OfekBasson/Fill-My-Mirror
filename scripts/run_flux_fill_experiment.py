"""
Inpainting experiment script using FluxFillPipeline (FLUX.1-Fill-dev).

Downloads projection outputs from Cloudflare R2, runs FluxFillPipeline
...
Single GPU, single seed:
    ...
    python scripts/run_flux_fill_experiment.py \\
        --dataset mirrorbench_v2 \\
        --geom-subdir estimated_geometry \\
        --output-root /tmp/flux_fill_experiment \\
        --seeds 0
...
def _load_pipeline(gpu_id: int) -> FluxFillPipeline:
    device = f"cuda:{gpu_id}"
    pipe = FluxFillPipeline.from_pretrained(MODEL_NAME, torch_dtype=torch.bfloat16)
    pipe.to(device)
    return pipe
...
def _worker(
    ...
    max_sequence_length = args_dict["max_sequence_length"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run FLUX.1 Fill experiment from R2 projection outputs")
